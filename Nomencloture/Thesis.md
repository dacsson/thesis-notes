...
Для работы с Sail-моделями в RICOVER используется библиотека **Isla** (isla-lib) [7]. Isla - это символическая машина, способная исполнять Sail-код и транслировать его в формальное представление. Isla-sail компилирует Sail-модель RISC-V в *Isla IR* - промежуточное представление на основе статической формы с единственным присваиванием (SSA). Именно этот файл `.ir` служит входными данными для модуля `isla_ir.rs` в RICOVER.

Ключевой шаг конвейера - *специализация* вариантов. Функция `execute` - это один обработчик на несколько вариантов через сравнение с образцом. Isla IR сохраняет эту структуру, а `isla_ir.rs` перебирает все ветки и специализирует каждую по конкретному значению `op`, получая отдельное CHC-правило на каждый вариант инструкции. Таким образом, одна Sail-функция `execute ITYPE` порождает шесть независимых CHC-правил: `addi`, `slti`, `sltiu`, `andi`, `ori`, `xori`.

Полный вывод в виде Isla IR для RISC-V 64 (файл `snapshot/rv64d.ir`) содержит 746 вариантов инструкций. Из них 210 успешно транслируются в CHC-правила, поддерживая расширеный профиль `rv64i`. 
...
Дизъюнкты Хорна естественно кодируют множество достижимых состояний последовательных программ: *выполнимые* дизъюнкты Хорна соответствуют свойствам программы, которые выполняются, тогда как *невыполнимые* - нарушенным свойствам. Это делает CHC канонической промежуточной формой для верификации программ.
...
Для решения CHC в настоящей работе применяется решатель Z3 Spacer [14].

### 2.4 Кодирование семантической эквивалентности в CHC

TODO: something here to start off this section
...
Команда `translate-ir` генерирует для каждой инструкции отдельное CHC-отношение (`declare-rel` + `rule`), как показано в листинге 3.4. При формировании запроса эквивалентности (`check-equiv`) тело каждого блока могло бы вызывать эти отношения:
...
Вместо этого `check-equiv` применяет прямое встраивание: тело каждого правила блока содержит три равенства-ограничения непосредственно, без ссылки на инструкцию в виду отношения. Такой подход сокращает глубину CHC-системы с двух уровней (инструкционные отношения, программные отношения) до одного (только блочные и композиционные отношения), что упрощает задачу поиска индуктивного инварианта алгоритмом Spacer.
...
Формирование SMT-LIB2-вывода сосредоточено в модуле `src/chc_emit/`, организованном в виде восьми специализированных подмодулей (листинг 3.8).
...
При разборе функции `check-equiv` ищет первую инструкцию вида `addi sp, sp, -N` (или `c.addi sp, -N`). Число $N$ является размером стекового фрейма функции. Приватный фрейм определяется как интервал $[\text{sp}_0 - N, \text{sp}_0)$, где $\text{sp}_0$ - значение указателя стека в начальном состоянии.

CHC-запрос `bad` эмитируется как два правила, реализующих формулы (2.8) и (2.9), показанные в листинге 3.7.
