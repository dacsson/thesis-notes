...
Таким образом, для формальной верификация нужен только перебор программ, и что самое главное - она даёт математическую гарантию того, что оптимизация корректна для всех возможных входных данных.
...
Над циклами в программе применяется много оптимизаций, например `loop-unroll` (размотка цикла) или `licm` (вынос инвариантного вычисления). При этом в самой программе содержится цикл с обратными рёбрами CFG. Закодировать такую программу напрямую в виде бесквантороной SMT-формулы нельзя, поэтому необходимо выполнить некоторые действия:

+ *Раскрутка цикла* --- развернуть цикл до некого фиксированного числа $k$ итераций. Однако, метод неполон: программа, расходящаяся на $k+1$ итерации, останется необнаруженной. Кроме того, размер формулы растёт линейно с $k$.

+ *Квантификация* --- добавить кванторы в формулу. Однако бесквантороная теория (QF\_BV), на которой работают эффективные SMT-решатели, не поддерживает кванторы, а переход к полной логике первого порядка делает задачу неразрешимой или экспоненциально сложной.

+ *Ручное задание инварианта* --- аннотировать каждый цикл инвариантом вручную. Но, это ломает автоматизацию и неприменимо к коду произвольного компилятора.

Цикл в ассемблерном коде --- это базовый блок с обратным ребром CFG. В CHC-кодировании в инструменте RICOVER (раздел~3.6) такой блок генерирует *рекурсивное* правило:

#block(inset: (left: 1.25cm))[
  `f_from_bbN(σ_in, σ_out)` $<-$ `f_bbN(σ_in, σ_mid), cond(σ_mid), f_from_bbN(σ_mid, σ_out)`
]

Это правило говорит нам: «начав итерацию в состоянии $sigma_"in"$, выполнив тело блока, при выполнении условия продолжаем с нового состояния $sigma_"mid"$». CHC-решатель (в нашем случае Spacer @cite14) ищет некий *индуктивный инвариант* --- предикат $"Inv"(sigma)$, замкнутый относительно этого правила. Найденный инвариант является математическим доказательством корректности для *всех* итераций цикла, а не только для первых $k$.
