...
Результатом шага 3 является CHC-правило для каждой инструкции. Листинг @listing-addi-chc демонстрирует правило для `addi rd, rs1, imm`, полученное из Sail фунции `execute ITYPE` через Isla IR.

#figure(
  caption: [Транслированное CHC-правило инструкции addi (в формате SMT-LIB2)],
  raw(
    block: true,
    lang: none,
    "(declare-rel addi
...
Читать это правило следует так: если мы начали из состояния `(regs0, mem0, pc0)`, то исполнение `addi` с операндами `imm=p0, rs1=p1, rd=p2` приводит к состоянию `(regs1, mem1, pc1)`, где регистр `rd` получает значение `rs1 + sign_extend(imm)`. Память не изменяется, а PC увеличивается на 4. Тело правила было сгенерировано инструментом из Sail функции через Isla IR --- оно не написано вручную.

Команда `translate-ir` генерирует для каждой инструкции отдельное отношение (`declare-rel` + `rule`), как показано в листинге @listing-addi-chc. Вместо обращения к каждому сгенерированному отношению, шаг `check-equiv` применяет прямое встраиване, то есть, тело каждого правила блока содержит три равенства-ограничения непосредственно, без ссылки на инструкцию в виде отношения. Такой подход сокращает глубину системы с двух уровней (инструкционные отношения, программные отношения) до одного (только блочные и композиционные отношения), что упрощает задачу поиска индуктивного инварианта алгоритмом Spacer.
...
Функция `instruction_to_chc` сопоставляет каждую инструкцию с именем соответствующего сгенерированного правила и списоком операндов. Порядок операндов определяется порядком полей в кортеже Sail для каждого класса инструкций (листинг @listing-asm-emit).

#figure(
  caption: [Отображение ассемблерных инструкций в SMT-операнды],
  ```rust
  match op {
...
где $n$ --- количество инструкций функции, а каждая тройка $("regs"_i, "mem"_i, "pc"_i)$ --- это промежуточное состояние машины после $i$-й инструкции.
...
#figure(
  caption: [Операции get\_reg и set\_reg в stdlib (SMT-LIB2)],
  raw(
    block: true,
    lang: none,
...
== Модель наблюдаемого состояния

Приведённая ранее формулировка проекционной эквивалентности $tilde.eq_"proj"$ (формула~2.7) требует точного определения множества $"ObsMem"(sigma_0)$ --- наблюдаемой памяти.
