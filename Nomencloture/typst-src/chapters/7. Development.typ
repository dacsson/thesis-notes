...
Точка входа RICOVER определяет два подкоманды через библиотеку аргументов командной строки Clap. Листинг @listing-cli демонстрирует их объявление.

#figure(
  caption: [Перечисление команд CLI в main.rs (Rust)],
  ```rust
...
== Трансляция Sail IR в CHC (модуль translate-ir)
...
+ *Оптимизация: устранение переменных памяти.* Инструкции, не изменяющие память (арифметические, логические, пересылки регистров), не получают собственной переменной `memN`. Вместо этого используется предыдущая переменная `memK`, устраняя тривиальные ограничения вида `(= memN memN-1)` над массивами.
...
== Разбор RISC-V ассемблера и формирование запроса (check-equiv)
...
Функция `instruction_to_chc` переводит каждую инструкцию в имя соответствующего CHC-правила и список SMT-операндов. Порядок операндов определяется порядком полей в кортеже Sail для каждого класса инструкций (листинг @listing-asm-emit).
...
== Стандартная библиотека CHC (stdlib.smt2)

Стандартная библиотека определяет примитивы, общие для всех сгенерированных запросов. Операции над регистрами реализованы как SMT-функции с учётом особенности нулевого регистра `x0` архитектуры RISC-V (листинг @listing-stdlib).
...
Для каждого базового блока `bb<N>` функции `f` эмитируются:

+ *Тело блока* --- `f_bb<N>(state_in, state_out)`: цепочка инлайн-ограничений для инструкций блока (без инструкции-терминатора);
+ *Композиция* --- `f_from_bb<N>(state_in, state_out)`: описывает все пути выполнения от входа в блок `bb<N>` до завершения функции. Для входного блока (`bb0`) эта функция именуется просто `f` и является функциональным резюме всей функции.
