...
; Variant: riscv64-linux-gnu_+v,+d_loop-vectorize_OUTLOOP
; Command: /home/artjom/Tools/llvm-project/build/bin/opt -mtriple riscv64-linux-gnu -mattr=+v,+d -tail-folding-policy=dont-fold-tail -passes=loop-vectorize -S
; Original: RUN: opt -mtriple riscv64-linux-gnu -mattr=+v,+d -tail-folding-policy=dont-fold-tail -passes=loop-vectorize < %s -S -o - | FileCheck %s -check-prefix=OUTLOOP
...
; ModuleID = '/tmp/tmpm8yp953a.ll'
source_filename = "/tmp/tmpm8yp953a.ll"
target datalayout = "e-m:e-p:64:64-i64:64-i128:128-n64-S128"
...
vector.body:                                      ; preds = %vector.body, %vector.ph
  ...
  %wide.load = load <vscale x 4 x i16>, ptr %4, align 2
  %5 = sext <vscale x 4 x i16> %wide.load to <vscale x 4 x i32>
  %6 = add <vscale x 4 x i32> %vec.phi, %5
...
scalar.ph:                                        ; preds = %for.body.preheader, %middle.block
  ...
  %bc.merge.rdx = phi i32 [ %8, %middle.block ], [ 0, %for.body.preheader ]
  br label %for.body
...
for.cond.cleanup.loopexit:                        ; preds = %middle.block, %for.body
  %add.lcssa = phi i32 [ %add, %for.body ], [ %8, %middle.block ]
...
vector.body:                                      ; preds = %vector.body, %vector.ph
  ...
  %vec.phi = phi <vscale x 4 x i32> [ %broadcast.splat, %vector.ph ], [ %6, %vector.body ]
  %4 = getelementptr inbounds i32, ptr %a, i64 %index
  %wide.load = load <vscale x 4 x i32>, ptr %4, align 4
  %5 = icmp slt <vscale x 4 x i32> %wide.load, %vec.phi
  %6 = select <vscale x 4 x i1> %5, <vscale x 4 x i32> %wide.load, <vscale x 4 x i32> %vec.phi
  %index.next = add nuw i64 %index, %3
  %7 = icmp eq i64 %index.next, %n.vec
  br i1 %7, label %middle.block, label %vector.body, !llvm.loop !4

middle.block:                                     ; preds = %vector.body
  %8 = call i32 @llvm.vector.reduce.smin.nxv4i32(<vscale x 4 x i32> %6)
...
for.body:                                         ; preds = %scalar.ph, %for.body
  ...
  %cmp.i = icmp slt i32 %9, %rdx
  %smin = select i1 %cmp.i, i32 %9, i32 %rdx
  %iv.next = add nuw nsw i64 %iv, 1
  %exitcond.not = icmp eq i64 %iv.next, %n
  br i1 %exitcond.not, label %for.end, label %for.body, !llvm.loop !5

for.end:                                          ; preds = %middle.block, %for.body
  %smin.lcssa = phi i32 [ %smin, %for.body ], [ %8, %middle.block ]
  ret i32 %smin.lcssa
}
