; Source: /home/artjom/Tools/llvm-project/llvm/test/Transforms/LoopVectorize/RISCV/inloop-reduction.ll
; Variant: riscv64-linux-gnu_+v,+d_loop-vectorize_INLOOP
; Command: /home/artjom/Tools/llvm-project/build/bin/opt -mtriple riscv64-linux-gnu -mattr=+v,+d -tail-folding-policy=dont-fold-tail -passes=loop-vectorize -prefer-inloop-reductions -S
; Original: RUN: opt -mtriple riscv64-linux-gnu -mattr=+v,+d -tail-folding-policy=dont-fold-tail -passes=loop-vectorize -prefer-inloop-reductions < %s -S -o - | FileCheck %s -check-prefix=INLOOP
...
; ModuleID = '/tmp/tmpywbrsgkt.ll'
source_filename = "/tmp/tmpywbrsgkt.ll"
target datalayout = "e-m:e-p:64:64-i64:64-i128:128-n64-S128"
target triple = "riscv64-unknown-linux-gnu"

define i32 @add_i16_i32(ptr readonly captures(none) %x, i32 %n) #0 {
entry:
  %cmp6 = icmp sgt i32 %n, 0
...
for.body.preheader:                               ; preds = %entry
  ...
  %1 = shl nuw i32 %0, 3
  %umax = call i32 @llvm.umax.i32(i32 %1, i32 16)
...
vector.body:                                      ; preds = %vector.body, %vector.ph
  ...
  %4 = getelementptr inbounds i16, ptr %x, i32 %index
  %wide.load = load <vscale x 8 x i16>, ptr %4, align 2
  %5 = sext <vscale x 8 x i16> %wide.load to <vscale x 8 x i32>
  %6 = call i32 @llvm.vector.reduce.add.nxv8i32(<vscale x 8 x i32> %5)
  %7 = add i32 %vec.phi, %6
...
scalar.ph:                                        ; preds = %for.body.preheader, %middle.block
  %bc.resume.val = phi i32 [ %n.vec, %middle.block ], [ 0, %for.body.preheader ]
  %bc.merge.rdx = phi i32 [ %7, %middle.block ], [ 0, %for.body.preheader ]
  br label %for.body

for.body:                                         ; preds = %scalar.ph, %for.body
  %i.08 = phi i32 [ %inc, %for.body ], [ %bc.resume.val, %scalar.ph ]
  %r.07 = phi i32 [ %add, %for.body ], [ %bc.merge.rdx, %scalar.ph ]
  %arrayidx = getelementptr inbounds i16, ptr %x, i32 %i.08
  ...
  %inc = add nuw nsw i32 %i.08, 1
  %exitcond = icmp eq i32 %inc, %n
  br i1 %exitcond, label %for.cond.cleanup.loopexit, label %for.body, !llvm.loop !3
...
for.cond.cleanup:                                 ; preds = %for.cond.cleanup.loopexit, %entry
  %r.0.lcssa = phi i32 [ 0, %entry ], [ %add.lcssa, %for.cond.cleanup.loopexit ]
  ret i32 %r.0.lcssa
}
...
vector.body:                                      ; preds = %vector.body, %vector.ph
  ...
  %5 = call i32 @llvm.vector.reduce.smin.nxv4i32(<vscale x 4 x i32> %wide.load)
  %rdx.minmax = call i32 @llvm.smin.i32(i32 %5, i32 %vec.phi)
...
for.body:                                         ; preds = %scalar.ph, %for.body
  ...
  %cmp.i = icmp slt i32 %7, %rdx
  %smin = select i1 %cmp.i, i32 %7, i32 %rdx
  %iv.next = add nuw nsw i64 %iv, 1
  %exitcond.not = icmp eq i64 %iv.next, %n
...
; Function Attrs: nocallback nocreateundeforpoison nofree nosync nounwind speculatable willreturn memory(none)
declare i32 @llvm.vector.reduce.smin.nxv4i32(<vscale x 4 x i32>) #2
