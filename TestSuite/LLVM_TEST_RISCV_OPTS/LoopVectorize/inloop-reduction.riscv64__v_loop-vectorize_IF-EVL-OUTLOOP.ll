...
; Original: RUN: opt -passes=loop-vectorize -tail-folding-policy=prefer-fold-tail -mtriple=riscv64 -mattr=+v -S < %s 2>&1 | FileCheck --check-prefix=IF-EVL-OUTLOOP %s
...
; ModuleID = '/tmp/tmp5ocmh6sj.ll'
source_filename = "/tmp/tmp5ocmh6sj.ll"
target datalayout = "e-m:e-p:64:64-i64:64-i128:128-n64-S128"
target triple = "riscv64"
...
vector.body:                                      ; preds = %vector.body, %vector.ph
  ...
  %vec.phi = phi <vscale x 4 x i32> [ zeroinitializer, %vector.ph ], [ %4, %vector.body ]
  ...
  %vp.op.load = call <vscale x 4 x i16> @llvm.vp.load.nxv4i16.p0(ptr align 2 %1, <vscale x 4 x i1> splat (i1 true), i32 %0)
  %2 = sext <vscale x 4 x i16> %vp.op.load to <vscale x 4 x i32>
  %3 = add <vscale x 4 x i32> %vec.phi, %2
...
for.cond.cleanup:                                 ; preds = %for.cond.cleanup.loopexit, %entry
  %r.0.lcssa = phi i32 [ 0, %entry ], [ %6, %for.cond.cleanup.loopexit ]
  ret i32 %r.0.lcssa
}
...
declare <vscale x 4 x i16> @llvm.vp.load.nxv4i16.p0(ptr captures(none), <vscale x 4 x i1>, i32) #2
