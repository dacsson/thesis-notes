...
; Variant: loop-vectorize_CHECK-UNORDERED
; Command: /home/artjom/Tools/llvm-project/build/bin/opt -passes=loop-vectorize -S
; Original: RUN: opt -passes=loop-vectorize -S < %s | FileCheck %s --check-prefix=CHECK-UNORDERED
...
; ModuleID = '/tmp/tmpa9srjw01.ll'
source_filename = "/tmp/tmpa9srjw01.ll"
...
vector.body:                                      ; preds = %vector.body, %vector.ph
  ...
  %vec.phi = phi <4 x float> [ <float 0.000000e+00, float -0.000000e+00, float -0.000000e+00, float -0.000000e+00>, %vector.ph ], [ %1, %vector.body ]
  %0 = getelementptr inbounds float, ptr %a, i64 %index
  %wide.load = load <4 x float>, ptr %0, align 4
  %1 = fadd <4 x float> %wide.load, %vec.phi
  %index.next = add nuw i64 %index, 4
  %2 = icmp eq i64 %index.next, %n.vec
  br i1 %2, label %middle.block, label %vector.body, !llvm.loop !0

middle.block:                                     ; preds = %vector.body
  %3 = call float @llvm.vector.reduce.fadd.v4f32(float -0.000000e+00, <4 x float> %1)
...
scalar.ph:                                        ; preds = %entry, %middle.block
  ...
  %bc.merge.rdx = phi float [ %3, %middle.block ], [ 0.000000e+00, %entry ]
  br label %for.body
...
for.body:                                         ; preds = %scalar.ph, %for.body
  ...
  %add = fadd float %4, %sum.07
  %iv.next = add nuw nsw i64 %iv, 1
  %exitcond.not = icmp eq i64 %iv.next, %n
  br i1 %exitcond.not, label %for.end, label %for.body, !llvm.loop !3

for.end:                                          ; preds = %middle.block, %for.body
  %add.lcssa = phi float [ %add, %for.body ], [ %3, %middle.block ]
  ret float %add.lcssa
}
