...
; Command: /home/artjom/Tools/llvm-project/build/bin/opt -passes=loop-vectorize -force-target-max-vector-interleave=1 -mtriple=riscv64 -mattr=+v -scalable-vectorization=off -riscv-v-register-bit-width-lmul=2 -S
; Original: RUN: opt < %s -passes=loop-vectorize -force-target-max-vector-interleave=1 -mtriple=riscv64 -mattr=+v -scalable-vectorization=off -riscv-v-register-bit-width-lmul=2 -S | FileCheck %s --check-prefix=LMUL2
...
; ModuleID = '/tmp/tmpq2e3m7tr.ll'
source_filename = "/tmp/tmpq2e3m7tr.ll"
