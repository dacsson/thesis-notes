...
; Variant: riscv64_+zve32x_vector-combine_ZVE32X
; Command: /home/artjom/Tools/llvm-project/build/bin/opt -mtriple=riscv64 -mattr=+zve32x -passes=vector-combine -S
; Original: RUN: opt -S -mtriple=riscv64 -mattr=+zve32x %s -passes=vector-combine | FileCheck %s --check-prefix=ZVE32X
...
; ModuleID = '/tmp/tmp3ce0sekt.ll'
source_filename = "/tmp/tmp3ce0sekt.ll"
...
attributes #0 = { "target-features"="+zve32x" }
attributes #1 = { nocallback nofree nosync nounwind speculatable willreturn memory(none) "target-features"="+zve32x" }
attributes #2 = { nocallback nofree nosync nounwind willreturn memory(argmem: write) "target-features"="+zve32x" }
